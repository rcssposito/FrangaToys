...
// Função para gerar Payload PIX Copia e Cola (Padrão EMVCo)
function generatePixPayload(key: string, name: string, amount: number) {
    ...
    payload += "26330014br.gov.bcb.pix"; // Merchant Account Information GUI
    ...
    payload += "62070503***"; // Additional Data Field Template (*** = Generated by PSP)
                                    ...
                                    <div className="bg-zinc-950 border border-emerald-500/30 p-4 rounded-xl space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                        ...
                                        {paymentMethod === 'pix' && (
                                            <div className="bg-white p-4 rounded-xl flex flex-col items-center gap-3">
                                                <QRCodeSVG
                                                    value={generatePixPayload("43687871886", "Renan C S Sposito", totalVenda)}
                                                    size={160}
                                                    level="M"
                                                    includeMargin={true}
                                                />
                                                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest text-center">Peça pro cliente escanear<br />Valor: R$ {totalVenda.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                            </div>
                                        )}
                                        ...
                                        >
                                            {submitting ? <Loader2 className="animate-spin" /> : 'CONFIRMAR VENDA (PAGO)'}
                                        </button>
                                    </div>
                                )}
                            </div>
